...
#define REG_STANDBY_MODE               0x80
#define REG_STDBY_RC                       0x00
...
/**
 * Get Status of the Lora chip
 */
int lora_get_status(void)
{
   int status = 0;
   status = lora_read_reg(0xC0);
   return status;
}

uint32_t lora_get_irq_status(void)
{
   return(lora_read_reg(REG_IRQ_FLAGS));
}

void lora_set_irq_mask(void){
   lora_write_reg(0x08, 0b00000000|0b00000000|0b00000000|0b00000001|0b00000000|0b01111111);
}

void lora_set_standby_mode(void){
   lora_write_reg(REG_STANDBY_MODE, REG_STDBY_RC);
}
 ...
 */
int lora_init(int CS_PIN, int SCK_PIN, int MISO_PIN, int MOSI_PIN, int RST_PIN) {
   esp_err_t ret;
   lora_cs = CS_PIN;
   lora_clock = SCK_PIN;
   lora_miso = MISO_PIN;
    ...
    */
   lora_sleep();
   lora_write_reg(REG_FIFO_RX_BASE_ADDR, 0);//0
   lora_write_reg(REG_FIFO_TX_BASE_ADDR, 100);//0
...
void 
lora_send_short_packet(uint8_t *buf, int size)
{
   /*
    * Transfer data to radio.
    */
